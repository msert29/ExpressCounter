...
class Controller_Cart_Toppings(QDialog):
    ...
    def display_toppings(self, toppings_list):
        ...
        self.ui.display_custom_pizza_toppings_no(toppings_list)
        self.handle_custom_topping_selection(toppings_list)
        self.handle_custom_no_topping_selection(toppings_list)
    ...
    def handle_add_request(self, size, name, pizza_id):
        ...
        if (len(self.__custom_topping_list) > 0):
            ...
            # Can have extra toppings but remove some others so counter that condition
            if (len(self.__custom_no_topping_list) > 0):
                self.__no_toppings = " ".join(self.__custom_no_topping_list)
                self.__cat_toppings = self.__cat_toppings + ", " + self.__no_toppings
            # Now send it to the shopping list
            self.custom_pizza_inserted.emit("Pizza", size, name, price, self.__cat_toppings, pizza_id)
        elif (len(self.__custom_no_topping_list) > 0) and (len(self.__custom_topping_list) == 0):
                self.__cat_toppings = " ".join(self.__custom_no_topping_list)      
            # Now send it to the shopping list
                self.custom_pizza_inserted.emit("Pizza", size, name, float(price), str(self.__cat_toppings), pizza_id)
        else:
    ...
    def handle_custom_no_topping_selection(self, topping_list):
        toppings = self.ui.get_no_toppings()
        toppings[0].stateChanged.connect(lambda : self.set_no_topping(toppings[0].checkState(), topping_list[0]['name']))
        toppings[1].stateChanged.connect(lambda : self.set_no_topping(toppings[1].checkState(), topping_list[1]['name']))
        toppings[2].stateChanged.connect(lambda : self.set_no_topping(toppings[2].checkState(), topping_list[2]['name']))
        toppings[3].stateChanged.connect(lambda : self.set_no_topping(toppings[3].checkState(), topping_list[3]['name']))
        toppings[4].stateChanged.connect(lambda : self.set_no_topping(toppings[4].checkState(), topping_list[4]['name']))
        toppings[5].stateChanged.connect(lambda : self.set_no_topping(toppings[5].checkState(), topping_list[5]['name']))
        toppings[6].stateChanged.connect(lambda : self.set_no_topping(toppings[6].checkState(), topping_list[6]['name']))
        toppings[7].stateChanged.connect(lambda : self.set_no_topping(toppings[7].checkState(), topping_list[7]['name']))
        toppings[8].stateChanged.connect(lambda : self.set_no_topping(toppings[8].checkState(), topping_list[8]['name']))
        toppings[9].stateChanged.connect(lambda : self.set_no_topping(toppings[9].checkState(), topping_list[9]['name']))
        toppings[10].stateChanged.connect(lambda : self.set_no_topping(toppings[10].checkState(), topping_list[10]['name']))
        toppings[11].stateChanged.connect(lambda : self.set_no_topping(toppings[11].checkState(), topping_list[11]['name']))
        toppings[12].stateChanged.connect(lambda : self.set_no_topping(toppings[12].checkState(), topping_list[12]['name']))
        toppings[13].stateChanged.connect(lambda : self.set_no_topping(toppings[13].checkState(), topping_list[13]['name']))
    
    @pyqtSlot(int, str)
    def set_no_topping(self, state, selected_topping):
        if state == Qt.Checked:
            self.__custom_no_topping_list.append("No " + selected_topping)
        elif state == Qt.Unchecked:
            #print ("Removed" + selected_topping)
            rowNo = self.__custom_no_topping_list.index("No " + selected_topping)
            self.__custom_no_topping_list.pop(rowNo)
            # Clear concatenated list as well as we pass the value of this to shopping list
        else:
            QMessageBox.critical(None, "Invalid topping checkbox state!", "The topping checkbox is in an unkown state")