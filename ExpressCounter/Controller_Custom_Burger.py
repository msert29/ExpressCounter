...
import View_Custom_Burger_Generated

class Controller_Custom_Burger(QDialog):
    
    burger_added  = pyqtSignal(str, str, str, str, int)

    def __init__(self, burger_size, burger_name, button_no):
        super(Controller_Custom_Burger, self).__init__()
        self.ui = View_Custom_Burger_Generated.Ui_Dialog()
    ...
    def handle_combobox_list_events(self):
        ...
        self.ui.burger_sel.currentIndexChanged.connect(lambda : self.handle_combo_assg("Burger", self.ui.burger_sel.currentText()))
    ...
    def handle_combo_assg(self, combo_type, selection):
        if (combo_type == "Size"):
            self.burger_size = selection
        elif (combo_type == "Burger"):
            self.burger_name = selection
        else:
    ...
    def handle_add_request(self):
        ...
        if salad == "" or sauce == "":
            ...
            self.burger_added.emit(self.burger_size, self.burger_name, salad, sauce, self.button_no)
            self.accept()
